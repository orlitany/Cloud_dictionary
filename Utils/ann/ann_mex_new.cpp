...
ANNpointArray createAnnPointArray(const double *data, int d, int n)
{
	...
	//ANNpointArray *pts_ptr = new ANNpointArray;
	//ANNpointArray &pts = *pts_ptr;
	//pts = annAllocPts(n, d);
...
{
	return mxUseBdTree ? mxIsLogicalScalarTrue(mxUseBdTree) : 0;
}
...
{
	return mxBucketSize ? *((const int*)mxGetData(mxBucketSize)) : 1;
}
...
{
	return mxSplitRule ? (ANNsplitRule)(*((const int*)mxGetData(mxSplitRule))) : ANN_KD_SUGGEST;        
}
...
{
	return mxShrinkRule ? (ANNshrinkRule)(*((const int*)mxGetData(mxShrinkRule))) : ANN_BD_SUGGEST;
}
...
{
	return mxSearchWay ? (ESearchScheme)(*((const int*)mxGetData(mxSearchWay))) : ANNK_STANDARD_SEARCH;
}
...
{
	return mxK ? *((const int*)mxGetData(mxK)) : 1;
}
...
{
	return mxErrBound ? *mxGetPr(mxErrBound) : 0;
}
...
{
	return mxSearchRadius ? *mxGetPr(mxSearchRadius) : 0;
}
...
ANNpointArray * createPointArray(const mxArray* mxPts, int& d, int& n)
{
    ...
    // create point array
	ANNpointArray *pts = new ANNpointArray;
	pts[0] = createAnnPointArray((double *)mxGetPr(mxPts), d, n);
                       ...
                       mxArray*& mxInds, mxArray*& mxDists)
{
    ...
    mxDists = mxCreateNumericMatrix(k, n, mxDOUBLE_CLASS, mxREAL);
        
    ANNpoint     q      = (ANNpoint)mxGetPr(mxQuery);        
    ANNidxArray  nn_idx = (ANNidxArray)mxGetData(mxInds);
    ANNdistArray dists  = (ANNdistArray)mxGetData(mxDists);
